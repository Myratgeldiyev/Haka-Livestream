...
export default function EpayBindScreen() {
	const insets = useSafeAreaInsets()
	const setBoundEpayId = usePaymentsStore(s => s.setBoundEpayId)
	const [epayAccount, setEpayAccount] = useState('')
	const [confirmEpayAccount, setConfirmEpayAccount] = useState('')

	const handleBack = useCallback(() => router.back(), [])
	const handleBind = useCallback(() => {
		const account = epayAccount.trim()
		const confirm = confirmEpayAccount.trim()
		if (!account || !confirm) {
			Alert.alert('Error', 'Please fill both fields')
			return
		}
		if (account !== confirm) {
			Alert.alert('Error', 'Accounts do not match')
			return
		}
		setBoundEpayId(account)
		router.back()
	}, [epayAccount, confirmEpayAccount, setBoundEpayId])
				...
				>
					...
					>
						...
						<TextInput
							...
							placeholder="Please enter"
							placeholderTextColor={PLACEHOLDER_COLOR}
							value={confirmEpayAccount}
							onChangeText={setConfirmEpayAccount}
							autoCapitalize="none"
							autoCorrect={false}
							keyboardType="default"
						/>
