...
def svga_to_webp(svga_path, output_path):
    try:
        ...
        if not frames:
            print(f"✗ No frames: {os.path.name(svga_path)}")
            return
        
        # Tüm frame'leri aynı boyuta getir
        w, h = frames[0].size
        resized = []
        for img in frames:
            if img.size != (w, h):
...
for svga in glob.glob("assets/emoji/emoji/NORMAL/*.svga"):
    name = os.path.splitext(os.path.basename(svga))[0]
    svga_to_webp(svga, f"assets/emoji/emoji/NORMAL/{name}.webp")

for svga in glob.glob("assets/emoji/emoji/vip/*.svga"):
    name = os.path.splitext(os.path.basename(svga))[0]
    svga_to_webp(svga, f"assetemoji/vip/{name}.webp")
